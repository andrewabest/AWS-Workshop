# We don't get executed from our own directory so we need to find out where our scripts have
# been dropped and execute everything from there.
$scriptPath = [System.IO.Path]::GetDirectoryName($PSScriptRoot)
Write-Host "Executing scripts from $scriptPath"

. $scriptPath\Scripts\Variables.ps1

Configuration ConfigureWebserver
{
    Import-DscResource -ModuleName cWinServer
	Import-DscResource -ModuleName cOctopus
	...
	Node 'localhost'
	{
		...
		{ 
			...
			ApiKey           = $BootstrapParameters['OctopusApiKey']
			OctopusServerUrl = $BootstrapParameters['OctopusServerUrl']
			Environments     = $BootstrapParameters['Environment']
			Roles            = $BootstrapParameters['OctopusRole']

			# Optional settings
			ListenPort       = 10933;
			DefaultApplicationDirectory = "C:\Octopus\Applications"
			DependsOn = "[xWebsite]LoadBalancerHealthCheckEndpoint"
		}
	}
}
